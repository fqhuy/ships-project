...
void ParallelACOPTVEstimator::Cluster() {
	...
	this->clusters0_ = new Matrix<int, int> (np, np);
	this->clusters1_ = new Matrix<int, int> (np, np);
	this->clusters01_ = new Matrix<int, int> (np, np);
...
Matrix<float, float>* ParallelACOPTVEstimator::Distances(
	...
	if(result->IsMapped())
		result->GetArray().GetMemoryModel().UnMap();
	...
	if(!f0->IsMapped())
		f0->GetArray().GetMemoryModel().Map();
	if(!f1->IsMapped())
		f1->GetArray().GetMemoryModel().Map();
	result->GetArray().GetMemoryModel().Map();
