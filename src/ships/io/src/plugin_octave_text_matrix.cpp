...
int OctaveTextMatrixReader::ReadAsFloatMatrix(const std::string& url, Matrix<
		float, float>*& matrix, MemoryModel<float>* memory_model,
		SampleModel<float, float>* sample_model) {
...
int OctaveTextMatrixReader::ReadAsInt32Matrix(const std::string& url, Matrix<
	...
	return this->Read<int32_t,int32_t>(url,matrix,memory_model,sample_model);
}
int OctaveTextMatrixReader::ReadAsInt16Matrix(const std::string& url, Matrix<
	...
	return this->Read<int16_t,int16_t>(url,matrix,memory_model,sample_model);
}
...
template<class T1, class T2> int OctaveTextMatrixReader::Read(
	...
	if (memory_model == NULL)
		memory_model = new MemoryModel<T1> (2, true, true, READ_WRITE);
	...
	matrix = new Matrix<T1, T2> (sizes[0], sizes[1], memory_model,sample_model);
